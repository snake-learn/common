...
/**
 * <p>
 * Ngoại lệ dùng để biểu thị các lỗi kiểm tra hợp lệ dữ liệu (validation) trong hệ thống.
 * </p>
 * <ul>
 *   <li>Thường được sử dụng khi dữ liệu đầu vào không hợp lệ hoặc không đáp ứng các ràng buộc nghiệp vụ.</li>
...
public class ValidationException extends BaseException {
     ...
     */
    private static final String TAG = ValidationException.class.getSimpleName();
     ...
     */
    public ValidationException(@NonNull AppErrorInfo info) {
        super(TAG);
     ...
     */
    public ValidationException(@NonNull AppErrorInfo info, Object... templateArgs) {
        super(TAG);
        this.statusCode = info.getStatusCode();
        this.key = info.getKey();
     ...
     */
    public ValidationException(@NonNull AppErrorInfo info, Throwable cause) {
        super(TAG, cause);
        this.statusCode = info.getStatusCode();
        this.key = info.getKey();
        this.message = I18nProvider.getMessage(key);
     ...
     * @param templateArgs Tham số truyền vào mẫu thông báo
     */
    public ValidationException(@NonNull AppErrorInfo info, Throwable cause, Object... templateArgs) {
        super(TAG, cause);
        this.statusCode = info.getStatusCode();
        this.key = info.getKey();
        this.message = I18nProvider.getMessage(key);
